...
const EditVednorUser = () => {
    ...
    const fetchVendorUser = async () => {
      try {
        const response = await axios.get(
          `${Base_Url}/panel-fetch-vendor-user-by-id/${id}`,
          {
            headers: {
  ...
  const onSubmit = async (e) => {
    ...
    const isFormValid = document.getElementById("editVendorUserForm").checkValidity();
    ...
    if (isFormValid) {
      ...
      try {
        const response = await axios.post(
          `${Base_Url}/panel-update-vendor-user/${id}?_method=PUT`,
          formData,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  <Layout>
      <div className="p-2 bg-gray-50 min-h-screen">
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-6 w-full">
              <form id="editVendorUserForm" autoComplete="off" onSubmit={onSubmit}>
